...
def evaluator(test_case_name, total_triedHypotheses, hypothesis, formulas, solutions, recorded_traceid):
    ...
    if correctness == True or len(hypothesis) == 0:
        ...
        if len(hypotheses) != 0:
            backup_hypotheses = [[[], hypotheses]]
            res_solutions,triedHypotheses = evaluateHypotheses(test_case_name, total_triedHypotheses, backup_hypotheses, formulas,solutions, recorded_traceid)
            print("SAT Solving Cost:" + str((endtime - starttime).seconds) + " Seconds")
...
def evaluateHypotheses(test_case_name,  total_triedHypotheses, hypotheses,formulas,solutions,  recorded_traceid):
    if len(hypotheses) == 0:
    ...
    while len(hypotheses) > 0:
        ...
        for index in range(len(to_inject_hypotheses[1])):
            ...
            for solution in solutions:
                if set(solution) <= set(sorted(base_tried_hypotheses + hypothesis)):
                    validate_solution_flags = True
                    break
            # (not total_triedHypotheses or hypothesis not in total_triedHypotheses)  and
            if sorted(base_tried_hypotheses + hypothesis) not in total_triedHypotheses and (set(hypothesis)<set(base_tried_hypotheses)) == False and validate_solution_flags == False:
        ...
        for index in range(len(to_inject_hypotheses[1])):
            ...
            if sorted(base_tried_hypotheses + hypothesis) not in total_triedHypotheses and (set(hypothesis)<set(base_tried_hypotheses)) == False and validate_solution_flags == False:
                ...
                if correctness == True:
                    ...
                    next_based_tried_hypotheses = sorted(list(set(base_tried_hypotheses + hypothesis)))
                    total_triedHypotheses.append(sorted(list(set(base_tried_hypotheses + hypothesis))))
                    hypotheses.append([next_based_tried_hypotheses, next_hypotheses])
                    ...
                    logger.info((print_str))
                    total_triedHypotheses.append(sorted(list(set(base_tried_hypotheses + hypothesis))))
                    solutions.append(sorted(list(set(base_tried_hypotheses + hypothesis))))
...
def concreteEvaluator(test_case_name, hypothesis, formulas, recorded_traceid):
    if len(hypothesis) == 0:
        ...
        blades_ids = []
        solutions = evaluator(test_case_name, total_triedHypotheses, hypothesis, formulas, solutions, recorded_traceid)
        return solutions, total_triedHypotheses
...
log_path = "./logs/BFS_" + datetime.now().strftime("%Y-%m-%d-%H:%M:%S") + "LDFI solve logs"
