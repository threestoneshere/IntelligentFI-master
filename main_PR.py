...
def sorted_with_pagerank(to_sort_hypotheses, page_rank_scores):
    hypotheses_scores = []
    if len(to_sort_hypotheses) == 0:
        return to_sort_hypotheses
...
def evaluator(test_case_name, base_triedHypotheses, total_triedHypotheses, hypothesis, formulas, solutions, blades_ids, recorded_traceid, parent_index,PR_scores):
    ...
    if correctness == True or len(hypothesis) == 0:
        ...
        if len(sorted_hypotheses) != 0:
            ...
            base_triedHypotheses = sorted_with_pagerank(list(set(base_triedHypotheses + hypothesis)),PR_scores)
            res_solutions,triedHypotheses = evaluateHypotheses(test_case_name, base_triedHypotheses, total_triedHypotheses, sorted_hypotheses,formulas,solutions, blades_ids, recorded_traceid,global_index - 1,PR_scores)
            if len(blades_ids) >= 1:
        ...
        logger.info((print_str))
        global_index = global_index + 1
        total_triedHypotheses.append(sorted_with_pagerank(list(set(base_triedHypotheses + hypothesis)),PR_scores))
        solutions.append(sorted_with_pagerank(list(set(base_triedHypotheses + hypothesis)),PR_scores))
...
def evaluateHypotheses(test_case_name, base_triedHypotheses, total_triedHypotheses, hypotheses,formulas,solutions, blades_ids, recorded_traceid,parent_index,PR_scores):
    if len(hypotheses) == 0:
    ...
    for index in range(len(hypotheses)):
        ...
        for solution in solutions:
            if set(solution) <= set(sorted_with_pagerank(base_triedHypotheses + hypothesis,PR_scores)):
                validate_solution_flags = True
                break
        if (not total_triedHypotheses or hypothesis not in total_triedHypotheses)  and sorted_with_pagerank(base_triedHypotheses + hypothesis,PR_scores) not in total_triedHypotheses and (set(hypothesis)<set(base_triedHypotheses)) == False and validate_solution_flags == False:
            print("tried:-+--", base_triedHypotheses)
            # temp_triedHypotheses = (triedHypotheses)
            sols,blades_ids = evaluator(test_case_name, temp_base_triedHypotheses, total_triedHypotheses, hypothesis ,formulas,solutions, blades_ids, recorded_traceid,parent_index,PR_scores)
            
            return evaluateHypotheses(test_case_name, base_triedHypotheses, total_triedHypotheses,  hypotheses[index+1:], formulas, sols, blades_ids, recorded_traceid,parent_index,PR_scores)
        else:
            return evaluateHypotheses(test_case_name, base_triedHypotheses, total_triedHypotheses,  hypotheses[index+1:], formulas, solutions, blades_ids, recorded_traceid,parent_index,PR_scores)
...
def concreteEvaluator(test_case_name, hypothesis, formulas, recorded_traceid, PR_scores):
    if len(hypothesis) == 0:
        ...
        blades_ids = []
        solutions,_ = evaluator(test_case_name, base_triedHypotheses, total_triedHypotheses, hypothesis, formulas, solutions, blades_ids, recorded_traceid,0,PR_scores)
        return solutions, total_triedHypotheses
...
log_path = "./logs/DFS_PR_" + datetime.now().strftime("%Y-%m-%d-%H:%M:%S") + "LDFI solve logs"
