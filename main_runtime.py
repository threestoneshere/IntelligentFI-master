...
def getNewClause(test_case_name, recorded_traceid):
    target, method = test_case(test_case_name)
    time.sleep(15)
    ts = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) 
    paths,traces = collect_trace(ts)
    new_paths, new_traces = filter_trace(target, method, paths,traces)
    svc_paths = extract_pod_path(new_paths,recorded_traceid)
...
def evaluator(test_case_name, base_triedHypotheses, total_triedHypotheses, hypothesis, formulas, solutions, blades_ids, recorded_traceid, parent_index):
    ...
    if correctness == True or len(hypothesis) == 0:
        ...
        if len(hypotheses) != 0:
            ...
            base_triedHypotheses = sorted(list(set(base_triedHypotheses + hypothesis)))
...
def evaluateHypotheses(test_case_name, base_triedHypotheses, total_triedHypotheses, hypotheses,formulas,solutions, blades_ids, recorded_traceid,parent_index):
    ...
    for index in range(len(hypotheses)):
        ...
        for solution in solutions:
            if set(solution) <= set(sorted(base_triedHypotheses + hypothesis)):
...
log_path = "./logs/" + datetime.now().strftime("%Y-%m-%d") + "LDFI solve logs"
